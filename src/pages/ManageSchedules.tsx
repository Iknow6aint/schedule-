...
function ManageSchedules() {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      setSchedules((prev) => prev.filter((schedule: Schedule) => schedule._id !== id));
    } catch (error) {
                    ...
                    {filteredSchedules.map((schedule) => (
                      <tr key={schedule._id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button onClick={() => handleEdit(schedule._id)} className="text-indigo-600 hover:text-indigo-900">
                            <Edit2 className="h-5 w-5" />
                          </button>
                          <button onClick={() => handleDelete(schedule._id)} className="text-red-600 hover:text-red-900 ml-4">
